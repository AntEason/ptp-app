...
public class Jcode2SessionUtil {
    ...
    public static String jscode2session(String appid, String secret, String code, String grantType) throws Exception {
        ...
        HttpGet httpGet = new HttpGet(WeChatUrl.JS_CODE_2_SESSION.getUrl() + "?appid=" + appid + "&secret=" + secret + "&js_code=" + code + "&grant_type=" + grantType);
...
//        httpPost.setEntity(new UrlEncodedFormEntity(params));//添加参数
    ...
    public static boolean getUnlimited(String accessToken, String scene, String page, Integer width, boolean autoColor, String fileName, boolean isHyaline) {
        boolean result=true;
        JSONObject params = new JSONObject();
...
//        params.put("line_color",lineColor);
        ...
        byte[] data = HttpUtil.post(WeChatUrl.GET_UNLIMITED.getUrl() + "?access_token=" + accessToken, params.toString());
        if(scene.length()>32){
            log.info("微信获取去参数： {}" ,new String(data));
            return false;
        }
        FileOutputStream outStream = null;
        String path = System.getProperty("user.dir") + "/QRCode/"+fileName;
        log.info("生成小程序码的路径：{}", path);
